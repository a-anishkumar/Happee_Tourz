...
import { LayoutDashboard, Package, MessageSquare, LogOut, BarChart3, Users, PlusCircle, Settings, ChevronRight } from 'lucide-react'
...
const AdminDashboard = () => {
    ...
    const stats = [
        ...
        { label: 'New Inquiries', value: 12, icon: <BarChart3 />, color: 'bg-emerald-500' },
    ]
                        ...
                        <div className="flex flex-col gap-6">
                            {packages.slice(0, 4).map(pkg => (
                                <div key={pkg.id} className="flex items-center gap-4 group">
                                    <img src={pkg.image} alt={pkg.title} className="w-16 h-12 object-cover rounded-lg group-hover:scale-105 transition-all shadow-md" />
                        ...
                        <div className="flex flex-col gap-6">
                            {testimonials.slice(0, 4).map(test => (
                                <div key={test.id} className="flex items-center gap-4 group">
                                    <img src={test.image} alt={test.name} className="w-10 h-10 object-cover rounded-full group-hover:scale-110 transition-all shadow-md" />
                                    <div className="flex flex-col flex-grow">
